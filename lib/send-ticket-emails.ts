...
export async function sendTicketEmails(orderId: string) {
  ...
  /*
   * Release claims that may have been left behind by an interrupted
   * server process.
   */
  ...
  const { error: staleClaimError } = await supabaseAdmin
    ...
    .lt("email_send_started_at", staleClaimThreshold);

  if (staleClaimError) {
    console.error(
      "Could not release stale ticket email claims:",
      staleClaimError
    );
  }
  ...
  if (order.status !== "paid") {
    throw new Error(
      "Ticket emails can only be sent for a paid order."
    );
  }
  ...
  const failures: TicketEmailFailure[] = [];
  ...
  for (const attendee of attendees) {
    ...
    if (!ticket) {
      ...
      failures.push({
        ...
        message: "No valid ticket found for attendee."
      });
    ...
    const { data: claimedTicket, error: claimError } =
      await supabaseAdmin
        .from("tickets")
        .update({
          email_send_started_at: claimTime
        })
        .eq("id", ticket.id)
        .eq("status", "valid")
        .is("email_sent_at", null)
    ...
    try {
      ...
      const emailResult = await resend.emails.send({
        ...
        subject:
          `${event?.name ?? "STAR Camp"} Ticket Confirmation`,
        html: `
          <div style="font-family: Inter, Arial, sans-serif; line-height: 1.6; color: #1f1f1f;">
            ...
            <p>
              ...
              <strong>${event?.name ?? "STAR Camp"}</strong>
              has been confirmed.
            </p>
            ...
            <p>You can also open your ticket here:</p>
      ...
      const { error: sentUpdateError } =
        await supabaseAdmin
          ...
          .eq("id", ticket.id)
          .eq("email_send_started_at", claimTime);
      ...
      /*
       * Release only this request's claim. The claimTime predicate
       * prevents one request from clearing another request's claim.
       */
      ...
      if (releaseError) {
        console.error(
          "Could not release ticket email claim:",
          {
            ticketId: ticket.id,
            error: releaseError
          }
